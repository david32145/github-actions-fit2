...
export const todoMock = {
  ...
  findAll: (todos?: Todo[]) => rest.get('http://localhost:3333/todos',
    (req, res, ctx) => res(ctx.json(todos || []))),
  doneTodo:
    rest.patch('http://localhost:3333/todos/:id/complete',
      (req, res, ctx) => res(ctx.json({}))),
  destroy:
    rest.delete('http://localhost:3333/todos/:id',
      (req, res, ctx) => res(ctx.json({})))
}
